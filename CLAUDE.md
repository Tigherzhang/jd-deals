...
- 2026-07-27: 修复三大问题：(1) 单字"桃"删出 `_food_kw`，润本驱蚊/爽身不再误分食品；(2) 价格验证虚假标记修复（`_page_price_checked` 标志位，默认值 True→False）；(3) 修复移动端登录判断（`sdtoken`+`pin` 替代错误的 `pt_key`）。新增 `verify_daily_result.py` 和 `jd_login.py`，核验脚本已集成到 daily_job.py 末尾自动执行。
- 2026-07-27: 选品评分优化 — 基于手动选品经验：(1) 新增品牌分（京东自有品牌+5，知名国货+4）；(2) 大包装/组合装+3分；(3) 折扣<10%扣5分，20-50%最优区间+3分；(4) 品类权重调整（水果28→32，母婴5→8）；(5) 19.9元黄金价格带+3分；(6) _food_kw 补充拌面/拌粉/椰子汁/电解质/电解质水；(7) 新增饮料/速食子品类加分
