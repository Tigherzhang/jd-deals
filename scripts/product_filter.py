...
_PRODUCT_KEYWORDS_BY_CAT = {
    ...
    "日用品": [
        ...
        "油烟机", "抽油烟机", "水龙头", "水槽", "地漏", "角阀",
        # 注："软管"已移除——它在牙膏套装标题末尾（如"...泵式2支+软管3支"），
        # 会导致rfind()将牙膏误判为"软管"，触发历史去重。真正卖软管的极少见。
    ...
    "食品": [
        ...
        "皮蛋", "咸鸭蛋", "卤蛋", "蛋挞", "蛋黄酥", "手抓饼", "火锅丸", "丸子", "鱼丸", "虾丸",
        ...
        "果脯", "蜜饯", "枸杞", "原浆", "滋补", "披萨", "蛋挞皮", "蛋糕", "饼干", "糕点", "面包",
...
def score_product(item):
    ...
    # === 健康/特色概念加分（2分）===
    # 用户手动选品偏好：无抗、0添加、无铅、厚切、鲜等健康概念
    health_kw = ["无抗", "有机", "0添加", "0蔗糖", "无铅", "厚切", "鲜", "软籽", "生榨",
                 "无激素", "高蛋白", "低脂", "无糖"]
    for kw in health_kw:
        if kw in title:
            score += 2
    ...
    # === 地域特色加分（1.5分）===
    # 用户偏好有地域标识的商品：云南、鄱湖、攀枝花等
    region_kw = ["云南", "蒙阴", "鄱湖", "攀枝花", "石林", "海南", "广西",
                 "突尼斯", "内蒙", "新疆", "大理", "昆明", "烟台"]
    for kw in region_kw:
        if kw in title:
            score += 1.5
