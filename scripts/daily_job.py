...
def main():
    ...
    for elite_id, name in channels.items():
        # 扩大候选池：食品12页，家居8页，其他4页
        max_page = 12 if elite_id == 27 else 8 if elite_id == 29 else 4
    ...
    # ====== 步骤2.5: 预过滤 - 移除历史SKU（确保选品新颖）======
    # 核心原则：每天都是新去寻找新商品，而不是从以前找到的商品里面去选
    # 必须在解析SKU之后执行，因为解析后SKU才准确
    from product_filter import load_history
    history = load_history()
    historical_skus = set(history.get("sku_ids", [])[-280:])
    dup_count = len([i for i in all_items if i.get('sku_id') in historical_skus])
    print(f"\n🔍 预过滤: 移除 {dup_count} 条历史商品")
